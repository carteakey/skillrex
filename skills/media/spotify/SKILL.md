...
Control the user's Spotify account when Spotify playback, device, queue, search, playlist, album, and library tools are available.
...
**`401 Unauthorized` after a retry** — refresh token revoked. Tell the user to re-run the agent's Spotify auth flow.
