...
for account in accounts:
    ...
    # Corrected order: subcommand first, then options
    result = subprocess.run([himalaya_cmd, 'envelope', 'list', '--account', account, '--page-size', '500', '--output', 'json'], capture_output=True, text=True)
    ...
    try:
        ...
        if start_idx != -1:
            ...
            for msg_id in to_move:
                # Corrected order: subcommand first, then options
                subprocess.run([himalaya_cmd, 'message', 'move', '--account', account, 'Newsletters', str(msg_id)])
                moved_count += 1
