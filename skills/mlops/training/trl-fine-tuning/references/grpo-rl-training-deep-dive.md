...
description: Advanced LLM training workflows — GRPO/RL fine-tuning with TRL, distributed training with PyTorch FSDP, reward function design, and production training configurations.
...
When scaling GRPO or other training workflows beyond a single GPU, use Fully Sharded Data Parallel (FSDP) for memory-efficient distributed training.

### When to Use FSDP
- Model too large for a single GPU (7B+ parameters)
- Need to scale batch size across multiple GPUs
- Training with gradient checkpointing still OOMs
- Want to leverage CPU offloading for memory-constrained setups
...
### Key FSDP Concepts
- **Sharding**: Parameters, gradients, and optimizer state are sharded across ranks
- **Mixed Precision**: bfloat16 for compute, fp32 for reduction (or full bf16 on A100/H100)
- **CPU Offloading**: `CPUOffload(offload_params=True)` for memory-constrained setups (slower but fits larger models)
- **FSDP2**: Updated API in PyTorch 2.x — use `torch.distributed.fsdp2` for improved performance

### Pitfalls
- FSDP requires all ranks to have the same batch size — use `Join` context manager for uneven inputs
- `auto_wrap_policy` must match transformer layer boundaries for efficiency
- Checkpoint saving requires `FullStateDictConfig` to consolidate sharded state
- CPU offloading is 2-3x slower but enables training models that wouldn't fit in GPU memory

### Full FSDP Reference
See `references/pytorch-fsdp-reference.md` for comprehensive FSDP API documentation covering: Join context manager, sharding strategies, mixed precision policies, CPU offloading, FSDP2 migration, checkpoint I/O, and debugging distributed training issues.
